...
export class ClientWorkoutPlanDTO {
   ...
   */
  static validateGetWorkoutsRequest(params: Record<string, unknown>): GetWorkoutsRequestDTO {
   ...
   */
  static validateCompleteDailyWorkoutRequest(body: Record<string, unknown>): CompleteDailyWorkoutRequestDTO {
    ...
    // Validate day format (should be a number between 1-7)
    ...
    const workout: IExerciseView[] = body.workout.map((exercise: unknown, index: number) => {
      if (typeof exercise !== 'object' || exercise === null) {
        throw new Error(`Workout exercise at index ${index} must be an object`);
      }

      const ex = exercise as Record<string, unknown>;
      
      // Validate required fields
      const name = ValidationUtil.validateString(ex.name, `workout[${index}].name`, 100);
      const instructions = ValidationUtil.validateString(ex.instructions, `workout[${index}].instructions`, 500);

      // Validate optional fields
      const sets = ex.sets ? ValidationUtil.validateString(ex.sets, `workout[${index}].sets`, 50) : undefined;
      const reps = ex.reps ? ValidationUtil.validateString(ex.reps, `workout[${index}].reps`, 50) : undefined;
      const rest = ex.rest ? ValidationUtil.validateString(ex.rest, `workout[${index}].rest`, 50) : undefined;
      const duration = ex.duration ? ValidationUtil.validateString(ex.duration, `workout[${index}].duration`, 50) : undefined;
   ...
   */
  static validateGetWorkoutReportRequest(query: Record<string, unknown>): GetWorkoutReportRequestDTO {
