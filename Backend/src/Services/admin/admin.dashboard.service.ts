...
export class AdminDashboardService implements IAdminDashboardService {
  ...
  async getStats(): Promise<IAdminDashboardStats> {
    const [totalTrainers, totalClients] = await Promise.all([
      (this._userRepo as any).model.countDocuments({ role: 'trainer' }),
      (this._userRepo as any).model.countDocuments({ role: 'client' }),
    ]);

    // Active clients: client personalization with planStatus Active
    const activeClients = await (this._userRepo as any).model.countDocuments({ role: 'client', /* populated via lookup not trivial; fallback uses clients count */ });

    // Monthly revenue from payment collections marked completed this month
    const now = new Date();
    const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
    const endOfMonth = new Date(now.getFullYear(), now.getMonth() + 1, 0, 23, 59, 59, 999);
    const payments = await this._paymentRepo.findAll({ paymentStatus: 'completed', createdAt: { $gte: startOfMonth, $lte: endOfMonth } } as any);
  ...
  async getRevenueTrends(monthsBack = 6): Promise<IRevenueTrendsResult> {
    ...
    for (let i = monthsBack - 1; i >= 0; i--) {
      ...
      const payments = await this._paymentRepo.findAll({ paymentStatus: 'completed', createdAt: { $gte: first, $lte: last } } as any);
      const sum = payments.reduce((acc: number, p: any) => acc + (p.amount || 0), 0);
      revenue.push(sum);
    }
  ...
  async getTopTrainers(limit = 5): Promise<ITopTrainerItem[]> {
    ...
    const completed = await this._paymentRepo.findAll({ paymentStatus: 'completed' } as any);
  ...
  async getRecentPayments(limit = 10) {
    const list = await this._paymentRepo.findAll({ paymentStatus: 'completed' } as any);
    const sorted = (list as any[]).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).slice(0, limit);
    // load names
    ...
    const [trainers, clients] = await Promise.all([
      (this._userRepo as any).model.find({ _id: { $in: trainerIds.map((id: string) => new Types.ObjectId(id)) } }, { _id: 1, name: 1 }),
      (this._userRepo as any).model.find({ _id: { $in: clientIds.map((id: string) => new Types.ObjectId(id)) } }, { _id: 1, name: 1 }),
    ]);
    const tMap = new Map<string, string>(trainers.map((t: any) => [t._id.toString(), t.name]));
    const cMap = new Map<string, string>(clients.map((c: any) => [c._id.toString(), c.name]));
    return sorted.map(p => ({
      id: p._id.toString(),
