...
export class TrainerDashboardService implements ITrainerDashboardService {
  ...
  async getStats(trainerId: string): Promise<ITrainerDashboardStats> {
    ...
    const activeClientIds = new Set(activeContracts.map((c: any) => c.clientId?.toString()));
    ...
    for (const contract of activeContracts) {
      const plan = await this._planRepo.findById((contract as any).planId?.toString());
      if (!plan) continue;
  ...
  async getTrends(trainerId: string): Promise<{ labels: string[]; sessions: number[]; activeClients: number[]; newClients: number[]; }> {
    ...
    for (let i = monthsBack - 1; i >= 0; i--) {
      ...
      const distinctActive = new Set(contractsInMonth.map((c: any) => c.clientId?.toString())).size;
      ...
      const distinctNew = new Set(newContracts.map((c: any) => c.clientId?.toString())).size;
  ...
  async getPayments(trainerId: string, options: { page?: number; limit?: number; status?: 'pending' | 'completed' | 'failed' | 'refunded' | 'all'; search?: string; }): Promise<{ payments: Array<{ id: string; clientId: string; amount: number; currency: string; paymentStatus: string; createdAt: string }>; total: number; page: number; limit: number; }> {
    ...
    const filtered = options.search ? all.filter((p: any) => p.clientId?.toString().includes(options.search as string)) : all;
    const total = filtered.length;
    const start = (page - 1) * limit;
    const slice = filtered.slice(start, start + limit);
    return {
      payments: slice.map((p: any) => ({ id: p._id.toString(), clientId: p.clientId?.toString(), amount: p.amount, currency: p.currency, paymentStatus: p.paymentStatus, createdAt: new Date(p.createdAt).toISOString() })),
      total,
      page,
      limit,
    };
