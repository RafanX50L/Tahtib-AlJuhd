...
exports.generateWorkoutReport = exports.generateExercisesForWeek = void 0;
exports.generateFitnessPlan = generateFitnessPlan;
const genai_1 = require("@google/genai");
const generative_ai_1 = require("@google/generative-ai");
const env_config_1 = require("..//config/env.config");
// Initialize the Google GenAI client
const genAI = new generative_ai_1.GoogleGenerativeAI(env_config_1.env.GEMINI_API_KEY);
async function generateFitnessPlan(userData, weekNumber, planType, previousWeekWorkoutData = null) {
    try {
...
function getMealCategories(mealsPerDay) {
    switch (mealsPerDay) {
        case "3 meals":
...
function formatPrompt(userData, weekNumber, planType, previousWeekWorkoutData = null) {
    const equipmentList = userData.equipment.length > 0
        ? userData.equipment.join(", ")
        : "bodyweight only";
    const healthConcerns = [...userData.healthIssues, userData.medicalCondition]
        .filter(Boolean)
        .join(", ");
    //   const dietRestriction =
    //     userData.dietAllergies.length > 0
    //       ? `Additionally, the user has allergies to: ${userData.diet_allergies.join(", ")}. `
    //       : "";
    ...
    const generateDayStructure = (currentWeekNum, dayNum, workoutDaysPerWeek) => {
    ...
    const generateSingleWeekStructure = (currentWeekNum, workoutDaysPerWeek) => {
        let days = "";
        for (let i = 1; i <= 7; i++) {
    ...
    if (planType === "workout" && weekNumber !== null) {
        ...
        if (weekNumber === 1) {
            progressionInstruction =
                "This is Week 1, so set a baseline difficulty and intensity.";
        }
        else {
      ...
      Desired workout duration: ${userData.workoutDuration} minutes per session
      Workout days per week: ${userData.workoutDaysPerWeek} days
      ...
      2. The **workout plan** should be ONLY for Week ${weekNumber}, with daily exercise routines based on the user's specified workout days per week (${userData.workoutDaysPerWeek}).
      ...
      8. The total duration of each workout session must be within ${userData.workoutDuration} minutes.
    ...
    `;
      ...
      {
        "week${weekNumber}": {
          ${generateSingleWeekStructure(weekNumber, userData.workoutDaysPerWeek)}
        },
    ...
    `;
      ...
      Target weight: ${userData.targetWeight} kg
      Fitness goal: ${userData.fitnessGoal}
      Diet allergies: ${userData.dietAllergies.join(", ") || "None"}
      Meals per day: ${userData.dietMealsPerDay}
      Diet preferences: ${userData.dietPreferences}
      
      IMPORTANT INSTRUCTIONS FOR GENERATING THE RESPONSE:
      1. The output MUST be a valid JSON object. Do NOT include any markdown formatting (like triple backticks) or additional explanatory text outside the JSON.
      2. The **diet plan** should align with the user's diet preferences (${userData.dietPreferences}) and allergies (${userData.dietAllergies.join(", ")}).
      3. Provide meal options for each meal category specified by ${userData.dietMealsPerDay}. For example, if ${userData.dietMealsPerDay} is "5", include breakfast, lunch, dinner, and two snack options.
...
// Initialize the Google GenAI client
const genAIS = new genai_1.GoogleGenAI({ apiKey: env_config_1.env.GEMINI_API_KEY });
const generateWorkoutReport = async (exercises) => {
...
exports.generateWorkoutReport = generateWorkoutReport;
const generateExercisesForWeek = async (type) => {
    try {
            ...
            const beginnerDummyPool = [
                {
                    ...
                    reps: "15 seconds",
                    rest: "30 seconds",
                    instructions: "Start with feet together, jump while spreading legs and arms, then return to start.",
                    animationLink: "https://example.com/jumping-jacks",
                },
                {
                    name: "Wall Push-Ups",
                    sets: 2,
                    reps: "12",
                    rest: "30 seconds",
                    instructions: "Stand facing a wall, place hands shoulder-width apart and perform a push-up.",
                    animationLink: "https://example.com/wall-pushups",
                },
                {
                    name: "Chair Squats",
                    sets: 2,
                    reps: "15",
                    rest: "30 seconds",
                    instructions: "Stand in front of a chair, lower until seated, then stand back up.",
                    animationLink: "https://example.com/chair-squats",
                },
                {
                    name: "Knee Lifts",
                    sets: 2,
                    reps: "30 seconds",
                    rest: "30 seconds",
                    instructions: "March in place lifting your knees to waist height.",
                    animationLink: "https://example.com/knee-lifts",
                },
                {
                    name: "Plank (Knees)",
                    sets: 2,
                    reps: "20 seconds",
                    rest: "30 seconds",
                    instructions: "Hold a plank from your knees and elbows.",
                    animationLink: "https://example.com/plank-knees",
                },
                {
                    name: "Leg Raises",
                    sets: 2,
                    reps: "15",
                    rest: "30 seconds",
                    instructions: "Lie on your back and raise your legs towards the ceiling.",
                    animationLink: "https://example.com/leg-raises",
                },
                {
                    name: "Bird Dog",
                    sets: 2,
                    reps: "10 each side",
                    rest: "30 seconds",
                    instructions: "On all fours, extend opposite arm and leg while keeping core stable.",
                    animationLink: "https://example.com/bird-dog",
                },
                {
                    name: "Glute Bridge",
                    sets: 2,
                    reps: "15",
                    rest: "30 seconds",
                    instructions: "Lie on your back, knees bent, lift hips off the ground.",
                    animationLink: "https://example.com/glute-bridge",
                },
                {
                    name: "Calf Raises",
                    sets: 2,
                    reps: "20",
                    rest: "30 seconds",
                    instructions: "Stand and raise up onto the balls of your feet.",
                    animationLink: "https://example.com/calf-raises",
                },
                {
                    name: "Arm Circles",
                    sets: 2,
                    reps: "20 seconds forward/backward",
                    rest: "30 seconds",
                    instructions: "Stand with arms extended, make small circles.",
                    animationLink: "https://example.com/arm-circles",
                },
                {
                    ...
                    rest: "30 seconds",
                    instructions: "Support yourself on one forearm and knees, keeping body straight.",
                    animationLink: "https://example.com/side-plank-knees",
                },
                {
                    name: "Toe Taps",
                    sets: 2,
                    reps: "30 seconds",
                    rest: "30 seconds",
                    instructions: "Alternately tap your toes to the ground in front of you.",
                    animationLink: "https://example.com/toe-taps",
                },
                {
                    ...
                    reps: "15",
                    rest: "30 seconds",
                    instructions: "Lie on back, knees bent, lift head and shoulders off floor.",
                    animationLink: "https://example.com/modified-crunches",
                },
                {
                    name: "Reverse Lunges",
                    sets: 2,
                    reps: "8 each leg",
                    rest: "30 seconds",
                    instructions: "Step backward into a lunge, then return to start.",
                    animationLink: "https://example.com/reverse-lunges",
                },
                {
                    ...
                    reps: "10",
                    rest: "30 seconds",
                    instructions: "Move spine from rounded to arched position on all fours.",
                    animationLink: "https://example.com/cat-cow",
                },
            ];
            ...
            const intermediateDummyPool = [
                {
                    ...
                    reps: "8-12",
                    rest: "45 seconds",
                    instructions: "Perform push-ups with hands shoulder-width apart, keeping body straight.",
                    animationLink: "https://example.com/standard-pushups",
                },
                {
                    ...
                    reps: "15-20",
                    rest: "45 seconds",
                    instructions: "Squat down as if sitting in a chair, keeping chest up and back straight.",
                    animationLink: "https://example.com/bodyweight-squats",
                },
                {
                    name: "Walking Lunges",
                    sets: 3,
                    reps: "10 each leg",
                    rest: "45 seconds",
                    instructions: "Step forward into a lunge, then continue forward with the opposite leg.",
                    animationLink: "https://example.com/walking-lunges",
                },
                {
                    name: "Plank Hold (Elbows)",
                    sets: 3,
                    reps: "45-60 seconds",
                    rest: "45 seconds",
                    instructions: "Hold body in a straight line from head to heels on your forearms.",
                    animationLink: "https://example.com/plank-elbows",
                },
                {
                    ...
                    reps: "15 each side",
                    rest: "45 seconds",
                    instructions: "Lie on back, bring opposite elbow to knee, mimicking cycling.",
                    animationLink: "https://example.com/bicycle-crunches",
                },
                {
                    name: "Triceps Dips (Bench/Chair)",
                    sets: 3,
                    reps: "12-15",
                    rest: "45 seconds",
                    instructions: "Use a sturdy bench or chair for dips, lowering your body by bending elbows.",
                    animationLink: "https://example.com/triceps-dips-bench",
                },
                {
                    name: "Step-Ups",
                    sets: 3,
                    reps: "10 each leg",
                    rest: "45 seconds",
                    instructions: "Step onto a sturdy elevated surface, focusing on the leg pushing up.",
                    animationLink: "https://example.com/step-ups",
                },
                {
                    name: "Russian Twists",
                    sets: 3,
                    ...
                    animationLink: "https://example.com/russian-twists",
                },
                {
                    name: "Push-Up to Plank Jack",
                    sets: 3,
                    reps: "10",
                    rest: "45 seconds",
                    instructions: "Perform a push-up, then jump feet out and in like a jumping jack in plank position.",
                    animationLink: "https://example.com/pushup-plank-jack",
                },
                {
                    name: "Side Plank",
                    sets: 3,
                    ...
                    rest: "45 seconds",
                    instructions: "Support yourself on one forearm and foot, keeping body straight.",
                    animationLink: "https://example.com/side-plank",
                },
                {
                    name: "Supermans",
                    sets: 3,
                    reps: "15",
                    rest: "30 seconds",
                    instructions: "Lie face down, lift arms and legs off the floor simultaneously.",
                    animationLink: "https://example.com/supermans",
                },
                {
                    name: "High Knees",
                    sets: 3,
                    reps: "45 seconds",
                    rest: "30 seconds",
                    instructions: "Run in place, bringing knees up towards your chest.",
                    animationLink: "https://example.com/high-knees",
                },
                {
                    name: "Butt Kicks",
                    sets: 3,
                    reps: "45 seconds",
                    rest: "30 seconds",
                    instructions: "Run in place, bringing heels towards your glutes.",
                    animationLink: "https://example.com/butt-kicks",
                },
                {
                    ...
                    reps: "15",
                    rest: "45 seconds",
                    instructions: "Lie on your back, lift your legs with knees bent, and curl your hips towards your chest.",
                    animationLink: "https://example.com/reverse-crunches",
                },
                {
                    ...
                    reps: "12-15",
                    rest: "45 seconds",
                    instructions: "Perform push-ups with hands elevated on a bench or sturdy surface.",
                    animationLink: "https://example.com/incline-pushups",
                },
            ];
            const advancedDummyPool = [
                {
                    ...
                    reps: "15",
                    rest: "45 seconds",
                    instructions: "Drop into push-up position, push-up, jump feet forward, leap up.",
                    animationLink: "https://example.com/burpees",
                },
                {
                    name: "Push-Ups",
                    sets: 3,
                    reps: "12-15",
                    rest: "45 seconds",
                    instructions: "Keep body straight, lower until chest nearly touches the ground, then push back up.",
                    animationLink: "https://example.com/pushups",
                },
                {
                    name: "Jump Squats",
                    sets: 3,
                    reps: "20",
                    rest: "45 seconds",
                    instructions: "Perform a squat, then explode up into a jump and land softly.",
                    animationLink: "https://example.com/jump-squats",
                },
                {
                    name: "Walking Lunges",
                    sets: 3,
                ...
                },
                {
                    ...
                    reps: "45 seconds",
                    rest: "45 seconds",
                    instructions: "Hold body in a straight line from head to heels.",
                    animationLink: "https://example.com/plank",
                },
                {
                    name: "Box Jumps",
                    sets: 3,
                    reps: "10",
                    rest: "60 seconds",
                    instructions: "Jump onto a sturdy box or elevated surface.",
                    animationLink: "https://example.com/box-jumps",
                },
                {
                    name: "Pull-Ups (Assisted/Negative)",
                    sets: 3,
                    reps: "6-8",
                    rest: "60 seconds",
                    instructions: "Use a band for assistance or focus on the lowering phase.",
                    animationLink: "https://example.com/pull-ups",
                },
                {
                    ...
                    reps: "5 each leg",
                    rest: "60 seconds",
                    instructions: "Use a support for balance while performing a single-leg squat.",
                    animationLink: "https://example.com/pistol-squats",
                },
                {
                    ...
                    reps: "45 seconds",
                    rest: "45 seconds",
                    instructions: "Rapidly bring knees to chest in a plank position.",
                    animationLink: "https://example.com/mountain-climbers",
                },
                {
                    ...
                    reps: "5-8",
                    rest: "60 seconds",
                    instructions: "Perform push-ups while in a handstand against a wall.",
                    animationLink: "https://example.com/handstand-pushups",
                },
                {
                    ...
                    reps: "10 each side",
                    rest: "45 seconds",
                    instructions: "From a side plank, reach your top arm under and through.",
                    animationLink: "https://example.com/side-plank-reach",
                },
                {
                    ...
                    reps: "10-12 reps",
                    rest: "45 seconds",
                    instructions: "Transition between high plank and forearm plank.",
                    animationLink: "https://example.com/commando-planks",
                },
                {
                    ...
                    reps: "20 each side",
                    rest: "45 seconds",
                    instructions: "Twist torso side-to-side while leaning back, holding a weight.",
                    animationLink: "https://example.com/russian-twists",
                },
                {
                    ...
                    reps: "15",
                    rest: "45 seconds",
                    instructions: "Use a chair for dips, lowering your body by bending elbows.",
                    animationLink: "https://example.com/triceps-dips",
                },
                {
                    ...
                    reps: "10 each leg",
                    rest: "60 seconds",
                    instructions: "Perform a lunge, then jump and switch legs in mid-air.",
                    animationLink: "https://example.com/plyo-lunges",
                },
            ];
            let exercisePool = [];
            if (type === "beginner") {
...
exports.generateExercisesForWeek = generateExercisesForWeek;
//# sourceMappingURL=gemini1.utils.js.map