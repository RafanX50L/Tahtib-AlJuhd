...
const WorkoutSession: React.FC = () => {
  ...
  const [videoId, setVideoId] = useState<string | null>(null); // Added state for videoId
  ...
  // Fetch YouTube video when a new exercise is loaded
  useEffect(() => {
    const fetchVideo = async () => {
      const exercise = workout[currentExerciseIndex];
      if (exercise?.name) {
        console.log("Searching YouTube for:", exercise.name); 
    ...
    if (workout.length > 0 && currentExerciseIndex < workout.length) {
      fetchVideo();
    }
  ...
  const loadExercise = useCallback(
    (index: number) => {
      ...
      setCurrentTime(
        metrics.type === "timed" || metrics.rangeType === "time" ? timeValue : 0
      );
      setTotalDuration(
        metrics.type === "timed" || metrics.rangeType === "time" ? timeValue : 0
      );
  ...
  const handleDurationSelect = useCallback(
    (duration: number) => {
      ...
      if (exercise) {
        ...
        setElapsedTime(0);
        const restSeconds = Number.parseInt(
          exercise?.rest?.split(" ")[0] || "0"
        );
        setRestTime(restSeconds);
        speakExercise(
          `Do ${exercise.name || "exercise"} for ${duration} seconds`
        );
        speakInstructionsWithDelay(exercise?.instructions || "");
      }
    },
    ...
    <main className="bg-[#12151E] text-white min-h-screen font-sans">
      ...
      <div className="max-w-3xl mx-auto p-8 flex flex-col animate-[fadeIn_0.6s_ease-out]">
        ...
        <div className="flex-1 flex flex-col items-center justify-center mb-4">
          <div className="w-full max-w-[560px] h-[315px] bg-gray-800 rounded-lg flex items-center justify-center border-4 border-blue-500 mb-6 overflow-hidden shadow-lg transition-transform hover:scale-105 hover:shadow-xl">
            {videoId ? (
              <iframe
                width="100%"
                height="100%"
          ...
          <div className="text-center">
            {metrics.type === "timed" ||
            (metrics.rangeType === "time" && customDuration !== null) ? (
        ...
        {/* Reselect Button */}
        {(metrics.rangeType === "reps" || metrics.rangeType === "time") &&
          !showRestScreen && (
              ...
              <Card className="bg-gray-800 border-gray-700 max-w-md w-full">
                ...
                <CardContent className="space-y-6">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {Array.from(
                      { length: metrics.range.max - metrics.range.min + 1 },
                      (_, i) => {
                        ...
                        return (
                          <Button
                            key={value}
                            variant={
                              selectedReps === value ? "default" : "outline"
                            }
                  ...
                  <div className="flex gap-3 pt-4">
                    <Button
                      variant="outline"
                      onClick={() => setShowRepSelectionModal(false)}
                      className="flex-1"
                    >
                      Cancel
                    </Button>
          ...
          <div className="fixed inset-0 bg-gray-900 bg-opacity-90 flex flex-col items-center justify-center z-50 p-4">
            <Card className="bg-gray-800 border-gray-700 max-w-md w-full">
              <CardHeader className="text-center">
                ...
                <p className="text-gray-400 mt-2">
                  Choose how long you want to do{" "}
              ...
              <CardContent className="space-y-6">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[
                    metrics.range.min,
                    Math.floor((metrics.range.min + metrics.range.max) / 2),
                    metrics.range.max,
                  ].map((value) => (
                    <Button
                      key={value}
        ...
        {isGeneratingReport && (
          <div className="fixed inset-0 bg-gray-900 bg-opacity-90 flex flex-col items-center justify-center z-50">
            ...
            <p className="text-gray-400">
              Please wait while we process your workout data
            </p>
          </div>
        )}
