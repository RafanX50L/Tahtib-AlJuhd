...
const APayments = () => {
  ...
  const fetchPayments = async (page: number, search: string) => {
    try {
      setLoading(true);
      setError(null);
      const response = await AdminService.getDashboardRecentPayments(page, pageSize, search);
            ...
            {loading ? (
              ...
              <div className="text-center py-16">
                ...
                <p className="text-gray-500">
                  {searchTerm ? "Try a different trainer name" : "Payment data will appear here once available"}
                </p>
              </div>
            ) : (
              <>
