...
import { User, Phone, MapPin, Clock, Calendar, Award, Video, Star, Edit3, Camera, Save } from "lucide-react";
...
interface TrainerProfileProps {
  ...
  setTrainerData: React.Dispatch<React.SetStateAction<ITrainerWithPersonalization | null>>;
}

const TrainerProfile: React.FC<TrainerProfileProps> = ({ trainerData, setTrainerData }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      // Send flat object with only changed fields
      const updateData = {
        ...(formData.name !== trainerData.name && { name: formData.name }),
        ...(formData.phoneNumber !== trainerData.basicInfo.phoneNumber && { phoneNumber: formData.phoneNumber }),
  ...
  const handlePictureUpdate = async () => {
    ...
    try {
      ...
      const response = await TrainerService.updateTrainerProfilePicture(formData);
      if (response) {
        setTrainerData(prev => prev ? ({
          ...prev,
          basicInfo: { ...prev.basicInfo, profilePhoto: URL.createObjectURL(profilePicFile) }
        }) : prev);
    ...
    <div className="bg-[#1e1e1e]/70 rounded-xl shadow-lg p-4 sm:p-6 lg:p-8 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col items-center mb-10">
        ...
        <div className="text-center mt-4">
          <h3 className="text-2xl font-bold text-white mb-2">{trainerData.name}</h3>
        ...
        {message && (
          <div className={`mt-4 px-6 py-3 rounded-lg text-sm font-medium ${
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="block text-[#ffffff]/80 font-medium mb-1">Name</label>
              <input
                type="text"
                value={formData.name}
                onChange={e => handleInputChange(e, "name")}
                className="w-full p-4 bg-[#2a2a2a] border border-[#404040] rounded-lg text-[#ffffff] focus:outline-none focus:border-[#6366f1] focus:ring-2 focus:ring-[#6366f1]/20 transition-all"
              />
            </div>
            <div className="space-y-2">
              <label className="block text-[#ffffff]/80 font-medium mb-1">Phone</label>
              <input
                type="text"
            ...
            <div className="space-y-2">
              <label className="block text-[#ffffff]/80 font-medium mb-1">Location</label>
              <input
                type="text"
          ...
          <div className="space-y-6">
            ...
            <div className="bg-[#2a2a2a]/50 p-6 rounded-xl border border-[#404040]/30">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h3 className="text-xl font-semibold text-[#ffffff]">Basic Information</h3>
              </div>
              <div className="space-y-4">
                <div className="flex items-center gap-4 p-3 bg-[#1a1a1a]/50 rounded-lg">
                  <Phone className="w-5 h-5 text-[#6366f1] flex-shrink-0" />
                  <div>
                    <span className="text-[#ffffff]/60 text-sm block">Phone</span>
                    <span className="text-white font-medium">{trainerData.basicInfo.phoneNumber}</span>
                  </div>
                </div>
                <div className="flex items-center gap-4 p-3 bg-[#1a1a1a]/50 rounded-lg">
                  <MapPin className="w-5 h-5 text-[#6366f1] flex-shrink-0" />
                  <div>
                    <span className="text-[#ffffff]/60 text-sm block">Location</span>
                    <span className="text-white font-medium">{trainerData.basicInfo.location}</span>
                  </div>
                </div>
                <div className="flex items-center gap-4 p-3 bg-[#1a1a1a]/50 rounded-lg">
                  <Clock className="w-5 h-5 text-[#6366f1] flex-shrink-0" />
                  <div>
                    <span className="text-[#ffffff]/60 text-sm block">Time Zone</span>
                    <span className="text-white font-medium">{trainerData.basicInfo.timeZone}</span>
                  </div>
                </div>
                {trainerData.basicInfo.dateOfBirth && (
                  <div className="flex items-center gap-4 p-3 bg-[#1a1a1a]/50 rounded-lg">
                    ...
                    <div>
                      <span className="text-[#ffffff]/60 text-sm block">Date of Birth</span>
                      <span className="text-white font-medium">
                        {new Date(trainerData.basicInfo.dateOfBirth).toLocaleDateString()}
                      </span>
                    </div>
                  </div>
                )}
                {trainerData.basicInfo.age && (
                  <div className="flex items-center gap-4 p-3 bg-[#1a1a1a]/50 rounded-lg">
                    ...
                    <div>
                      <span className="text-[#ffffff]/60 text-sm block">Age</span>
                      <span className="text-white font-medium">{trainerData.basicInfo.age} years</span>
                    </div>
                  </div>
                )}
                {trainerData.basicInfo.gender && (
                  <div className="flex items-center gap-4 p-3 bg-[#1a1a1a]/50 rounded-lg">
                    <User className="w-5 h-5 text-[#6366f1] flex-shrink-0" />
                    <div>
                      <span className="text-[#ffffff]/60 text-sm block">Gender</span>
                      <span className="text-white font-medium capitalize">{trainerData.basicInfo.gender}</span>
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className="bg-[#2a2a2a]/50 p-6 rounded-xl border border-[#404040]/30">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h3 className="text-xl font-semibold text-[#ffffff]">Sample Materials</h3>
              </div>
              <div className="space-y-4">
                ...
                {trainerData.sampleMaterials.portfolioLinks && trainerData.sampleMaterials.portfolioLinks?.length > 0 && (
                  <div className="p-4 bg-[#1a1a1a]/50 rounded-lg">
                    ...
                    <div className="flex flex-wrap gap-3">
                      {trainerData.sampleMaterials.portfolioLinks.map((link, index) => (
                        <a
                          key={index}
                          href={link}
                          target="_blank"
            ...
            <div className="bg-[#2a2a2a]/50 p-6 rounded-xl border border-[#404040]/30">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h3 className="text-xl font-semibold text-[#ffffff]">Availability</h3>
              </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-[#2a2a2a]/50 p-6 rounded-xl border border-[#404040]/30">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h3 className="text-xl font-semibold text-[#ffffff]">Professional Summary</h3>
              </div>
              <div className="space-y-6">
                <div className="p-4 bg-[#1a1a1a]/50 rounded-lg">
                  <div className="flex items-center justify-between">
                    <span className="text-[#ffffff]/70">Years of Experience</span>
                ...
                <div>
                  <h4 className="text-[#ffffff]/80 font-medium mb-3">Specializations</h4>
                  <div className="flex flex-wrap gap-2">
                    {trainerData.professionalSummary.specializations.map((spec, index) => (
                      <span 
                        key={index} 
                ...
                <div>
                  <h4 className="text-[#ffffff]/80 font-medium mb-3">Coaching Type</h4>
                  <div className="flex flex-wrap gap-2">
                    {trainerData.professionalSummary.coachingType.map((type, index) => (
                      <span 
                        key={index} 
                ...
                {trainerData.professionalSummary.platformsUsed?.length > 0 && (
                  <div>
                    <h4 className="text-[#ffffff]/80 font-medium mb-3">Platforms Used</h4>
                    <div className="flex flex-wrap gap-2">
                      {trainerData.professionalSummary.platformsUsed.map((platform, index) => (
                        <span 
                          key={index} 
                ...
                <div>
                  <h4 className="text-[#ffffff]/80 font-medium mb-3">Certifications</h4>
                  <div className="space-y-3">
                    {trainerData.professionalSummary.certifications.map((cert, index) => (
                      <div
                        key={index}
                      ...
                      >
                        ...
                        <span className="text-[#ffffff]/60 text-sm">Issued by {cert.issuer}</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            {trainerData.evaluation && (
              <div className="bg-[#2a2a2a]/50 p-6 rounded-xl border border-[#404040]/30">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <h3 className="text-xl font-semibold text-[#ffffff]">Performance Evaluation</h3>
                </div>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  {["communicationSkills", "technicalKnowledge", "coachingStyle", "confidencePresence", "brandAlignment", "equipmentQuality"].map((key, index) =>
                    trainerData.evaluation?.[key as keyof typeof trainerData.evaluation] && (
                      <div key={index} className="p-4 bg-[#1a1a1a]/50 rounded-lg">
                        <div className="flex flex-col gap-2">
                          ...
                          <div className="flex items-center gap-3">
                            <div className="flex">
                              {[1, 2, 3, 4, 5].map(star => (
                                <Star
                                  key={star}
                                  className={`w-4 h-4 ${
                                    star <= (trainerData.evaluation?.[key as keyof typeof trainerData.evaluation] as number)
                            ...
                            <span className="text-sm text-[#ffffff]/60 font-medium">
                              ({trainerData.evaluation?.[key as keyof typeof trainerData.evaluation]}/5)
                            </span>
                          </div>
                        </div>
                      </div>
                ...
                {trainerData.evaluation.notes && (
                  <div className="bg-[#1a1a1a]/50 p-5 rounded-lg mt-6 border border-[#404040]/30">
                    <h4 className="text-sm font-medium text-[#ffffff]/80 mb-3">Evaluation Notes</h4>
                    <p className="text-sm text-[#ffffff]/90 leading-relaxed">{trainerData.evaluation.notes}</p>
                  </div>
                )}
              </div>
            )}
  ...
  ), [trainerData, isEditing, formData, profilePicFile, tempProfilePic, message, loading, hasChanges]);
