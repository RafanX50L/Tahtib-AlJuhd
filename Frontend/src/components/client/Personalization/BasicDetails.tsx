...
const basicDetailsSchema = z.object({
  ...
  currentWeight: z.number().min(1, "Weight must be a positive number").optional(),
  targetWeight: z.number().min(1, "Target weight must be a positive number").optional(),
});
...
export default function BasicDetails() {
      ...
      setTimeout(() => {
        for (const [key, value] of Object.entries(formData)) {
          if (value !== undefined && value !== null) {
            form.setValue(key as keyof BasicDetailsForm, value, {
  ...
  useEffect(() => {
    ...
    if (formData[currentField] !== undefined) {
      form.setValue(currentField, formData[currentField]);
    } else {
