...
import { z } from "zod";
import { IClientUserData } from "@/services/implementation/clientServices";

const workoutPreferencesSchema = z.object({
  equipment: z.array(z.enum(["body weight", "dumbbells", "resistance bands", "kettlebells", "pull-up bar", "yoga mat"])).min(1, "Select at least one equipment type"),
  workoutDuration: z.enum(["15-30", "30-45", "45-60", "60+"]).optional(),
  workoutDaysPerWeek: z.number().min(1, "Must be at least 1 day").max(7, "Cannot exceed 7 days").optional(),
});
  ...
  const steps = [
    {
      ...
      type: "card",
      options: [
        { value: "body weight", icon: "👤", label: "Body Weight Only" },
        ...
        { value: "pull-up bar", icon: "🔼", label: "Pull-up Bar" },
        { value: "yoga mat", icon: "🧘", label: "Yoga Mat" },
      ],
    },
    {
      field: "workoutDuration" as const,
    ...
    {
      field: "workoutDaysPerWeek" as const,
  ...
  useEffect(() => {
    try {
      ...
      if (savedPrefs) {
        ...
        if (parsed.workoutDuration) parsedData.workoutDuration = parsed.workoutDuration;
        if (parsed.workoutDaysPerWeek) parsedData.workoutDaysPerWeek = parseInt(parsed.workoutDaysPerWeek);
  ...
  useEffect(() => {
    if (Object.keys(formData).length > 0) {
      Object.entries(formData).forEach(([key, value]) => {
        if (value !== undefined) {
          form.setValue(key as keyof WorkoutPreferencesForm, value , {
                      ...
                      <FormControl>
                        {currentFieldConfig.type === "card" ? (
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            {currentFieldConfig.options && currentFieldConfig.options.map((option) => (
                              <div
                                key={option.value}
                                className={cn(
                                  ...
                                  Array.isArray(field.value) && field.value.includes(option.value as IClientUserData["equipment"])
                                    ? "border-indigo-500 bg-gray-700/70 shadow-lg shadow-indigo-500/10"
                                    : "border-gray-700 bg-gray-800/50"
                                )}
                                onClick={() => {
                                  const currentEquipment = field.value || [];
                                  if (Array.isArray(currentEquipment) && currentEquipment.includes(option.value as IClientUserData["equipment"])) {
                                    field.onChange(currentEquipment.filter((item) => item !== option.value));
                                  } else if (Array.isArray(currentEquipment)) {
                              ...
                              >
                                ...
                                <div
                                  className={cn(
                                    ...
                                    Array.isArray(field.value) && field.value.includes(option.value as IClientUserData["equipment"]) ? "opacity-100" : "group-hover:opacity-50"
                                  )}
                                />
                              </div>
                            ))}
