...
const fitnessGoalsSchema = z.object({
  fitnessGoal: z.enum(["build muscle", "lose weight", "get stronger", "improve endurance", "tone body", "increase flexibility"]).optional(),
  currentFitnessLevel: z.enum(["beginner", "intermediate", "advanced", "athlete"]).optional(),
  activityLevel: z.enum(["sedentary", "lightly active", "moderately active", "very active"]).optional(),
});

type FitnessGoalsForm = z.infer<typeof fitnessGoalsSchema>;
type FitnessLevel = "beginner" | "intermediate" | "advanced" | "athlete" | undefined;
type activityLevel = "sedentary" | "lightly active" | "moderately active" | "very active" | undefined
...
export default function FitnessGoals() {
  ...
  useEffect(() => {
    try {
      ...
      const savedFitnessLevel = localStorage.getItem("currentFitnessLevel");
      ...
      if (savedFitnessLevel && fitnessGoalsSchema.shape.currentFitnessLevel.safeParse(savedFitnessLevel).success) {
        parsedData.currentFitnessLevel = savedFitnessLevel as FitnessLevel;
      }

      if (savedActivityLevel && fitnessGoalsSchema.shape.activityLevel.safeParse(savedActivityLevel).success) {
        parsedData.activityLevel = savedActivityLevel as activityLevel;
      }
  ...
  useEffect(() => {
    if (Object.keys(formData).length > 0) {
      Object.entries(formData).forEach(([key, value]) => {
        if (value !== undefined) {
          form.setValue(key as keyof FitnessGoalsForm, value, {
  ...
  const steps = [
    {
      ...
      type: "card",
      options: [
        { value: "build muscle", icon: "💪", label: "Build Muscle", description: "Increase muscle mass and definition" },
        { value: "lose weight", icon: "⚖️", label: "Lose Weight", description: "Burn fat and reduce body weight" },
        { value: "get stronger", icon: "🏋️", label: "Get Stronger", description: "Increase overall strength" },
        {
        ...
        { value: "tone body", icon: "👙", label: "Tone Body", description: "Define muscles without bulking up" },
        {
    ...
    {
      field: "currentFitnessLevel" as const,
    ...
    },
    {
      ...
      options: [
        ...
        { value: "lightly active", icon: "🚶", label: "Lightly Active", description: "Light exercise 1-3 days/week" },
        {
        ...
        { value: "very active", icon: "⚡", label: "Very Active", description: "Physical job or hard exercise 6-7 days/week" },
      ],
    },
  ];
    ...
    if (isValid) {
      ...
      if (currentStep < steps.length) {
        ...
        try {
          ...
          localStorage.setItem("currentFitnessLevel", data.currentFitnessLevel || "");
