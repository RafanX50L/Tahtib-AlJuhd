...
const dietPreferencesSchema = z.object({
  dietAllergies: z.array(z.enum(["none", "nuts", "dairy", "shellfish", "gluten"])).refine(
    (val) => !(val.includes("none") && val.length > 1),
    { message: "Cannot select 'None' with other allergies" }
  ).optional(),
  dietPreferences: z.string().optional(),
  dietMealsPerDay: z.enum(["3 meals", "3 meals + 1 snack", "3 meals + 2 snacks", "6 meals"]).optional(),
});

type DietPreferencesForm = z.infer<typeof dietPreferencesSchema>;
type Allergy = "none" | "nuts" | "dairy" | "shellfish" | "gluten";
...
export default function DietPreferences() {
  ...
  const steps = [
    ...
    {
      field: "dietPreferences" as const,
    ...
    {
      field: "dietMealsPerDay" as const,
  ...
  useEffect(() => {
    try {
      ...
      if (savedDietPrefs) {
        ...
        if (parsed.dietAllergies) parsedData.dietAllergies = parsed.dietAllergies;
        if (parsed.dietPreferences) parsedData.dietPreferences = parsed.dietPreferences.join(", ");
        if (parsed.dietMealsPerDay) parsedData.dietMealsPerDay = parsed.dietMealsPerDay;
  ...
  useEffect(() => {
    if (Object.keys(formData).length > 0) {
      Object.entries(formData).forEach(([key, value]) => {
        if (value !== undefined) {
          form.setValue(key as keyof DietPreferencesForm, value , {
  ...
  type dietAllergies = "none"| "nuts"| "dairy"| "shellfish" | "gluten";
  const handleSelectionToggle = (fieldName: "dietAllergies", value: dietAllergies) => {
  ...
  form.setValue(
    "dietMealsPerDay",
    value as "3 meals" | "3 meals + 1 snack" | "3 meals + 2 snacks" | "6 meals",
    { shouldValidate: true }
  );
};


  const handleDietPreferencesToggle = (value: string) => {
    const currentValue = form.getValues("dietPreferences") || "";
    if (value === "none") {
      form.setValue("dietPreferences", value, { shouldValidate: true });
    } else {
      if (currentValue === "none") {
        form.setValue("dietPreferences", value, { shouldValidate: true });
      } else {
        if (currentValue.includes(value)) {
          form.setValue("dietPreferences", "", { shouldValidate: true });
        } else {
          form.setValue("dietPreferences", value, { shouldValidate: true });
        }
      }
      ...
      if (currentStep < steps.length) {
        ...
        try {
          localStorage.setItem(
            "dietPreferences",
            JSON.stringify({
              dietAllergies: data.dietAllergies,
              dietPreferences: data.dietPreferences,
              dietMealsPerDay: data.dietMealsPerDay,
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <div className={cn(
                          "grid gap-4",
                          currentFieldConfig.field !== "dietMealsPerDay" && field.value?.includes("none") ? "grid-cols-1" : "grid-cols-1 sm:grid-cols-2"
                        )}>
                          {currentFieldConfig.options.map((option) => (
                            <div
                              ...
                              className={cn(
                                ...
                                option.value === "none" && currentFieldConfig.field !== "dietMealsPerDay" ? "sm:col-span-2" : "",
                                currentFieldConfig.field === "dietMealsPerDay"
                                  ? field.value === option.value
                                    ? "border-indigo-500 bg-gray-700/70 shadow-lg shadow-indigo-500/10"
                                    : "border-gray-700 bg-gray-800/50"
                                  : currentFieldConfig.field === "dietPreferences"
                                    ? field.value === option.value
                                      ? "border-indigo-500 bg-gray-700/70 shadow-lg shadow-indigo-500/10"
                                      : "border-gray-700 bg-gray-800/50"
                                    : field.value?.includes(option.value as Allergy )
                                      ? "border-indigo-500 bg-gray-700/70 shadow-lg shadow-indigo-500/10"
                                      : "border-gray-700 bg-gray-800/50"
                              )}
                              onClick={() =>
                                currentFieldConfig.field === "dietMealsPerDay"
                                  ...
                                  : currentFieldConfig.field === "dietPreferences"
                                    ? handleDietPreferencesToggle(option.value)
                                    : handleSelectionToggle(currentFieldConfig.field as "dietAllergies", option.value as dietAllergies)
                              }
                            >
                              {(currentFieldConfig.field === "dietMealsPerDay" ? field.value === option.value :
                                currentFieldConfig.field === "dietPreferences" ? field.value === option.value :
                                  field.value?.includes(option.value as Allergy)) && (
