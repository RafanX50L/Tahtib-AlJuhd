      ...
      <div className="flex items-center gap-6 mb-6">
        <p className="text-gray-400 text-sm">
          <span className="text-white font-medium">
            {data.workoutDuration} min/day
          </span>{" "}
          •{" "}
          <span className="text-white font-medium">
            {data.workoutDaysPerWeek} days/week
          </span>{" "}
        </p>
      </div>
      ...
      <div className="flex items-center gap-4 mb-6">
        <div className="h-2 bg-gray-700 rounded-full flex-grow overflow-hidden">
          <div
            ...
            style={{
              width: `${(data.workoutCompleted / (4 * data.workoutDaysPerWeek)) * 100}%`,
            }}
          ></div>
        </div>
        <div className="text-gray-400 text-sm">
          Day {data.workoutCompleted} of {7 * 4} • {"  "}
          {((data.workoutCompleted / (4 * 7)) * 100).toFixed(1)}% completed
        </div>
      </div>
    </div>
  );
