...
import { AvatarCstm, ButtonCstm, DialogContentCstm, DialogCstm, InputCstm } from "@/pages/common/community/CommunityApp";
...
const CommentModal: React.FC<CommentModalProps> = ({ post, posts, isOpen, onClose, onLikeToggle, onNavigateToPost, initialMediaIndex = 0 }) => {
...
const loadComments = async (loadMore = false, targetPost?: PostDTO) => {
  ...
  setIsLoading(true);
  try {
    const data = (await CommunityService.fetchComments(postToUse.id, loadMore ? cursor : undefined)).data;
    if (loadMore) {
      setComments(prev => [...prev, ...data.comments]);
    } else {
      setComments(data.comments || []);
    }
    // Since backend doesn't support pagination yet, set hasMore to false
    setHasMore(false);
    setCursor(undefined);
  } catch (error) {
...
// ... existing code ...
  ...
  const handleSubmitComment = async (e: React.FormEvent) => {
    ...
    try {
      const data = (await CommunityService.addComment(currentPost.id, newComment.trim())).data;
      console.log(data);
          ...
          <div className="flex-1 bg-gradient-to-br from-gray-900 to-black flex items-center justify-center relative">
            ...
            {(currentMediaIndex > 0 || currentIndex > 0) && (
              <ButtonCstm
                variant="ghost"
                size="sm"
                onClick={currentMediaIndex > 0 ? navigateToPreviousMedia : navigateToPreviousPost}
            ...
            {((currentPost && currentMediaIndex < currentPost.media.length - 1) || currentIndex < posts.length - 1) && (
              <ButtonCstm
                variant="ghost"
                size="sm"
                onClick={currentPost && currentMediaIndex < currentPost.media.length - 1 ? navigateToNextMedia : navigateToNextPost}
            ...
            <div className="absolute top-4 right-16 bg-black/50 text-white px-3 py-1 rounded-full text-sm">
              {currentPost.media.length > 1 && (
                <span>{currentMediaIndex + 1}/{currentPost.media.length}</span>
              )}
              {posts.length > 1 && (
          ...
          <div className="w-96 flex flex-col bg-white">
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <h4 className="font-bold text-gray-900">{currentPost.author?.name}</h4>
                  ...
                  <p className="text-sm text-gray-500">{new Date(currentPost.createdAt).toLocaleDateString()}</p>
                </div>
              </div>
              {currentPost.caption && (
                <p className="text-gray-800 leading-relaxed">{currentPost.caption}</p>
              )}
            </div>
            ...
            <div className="p-6 border-t border-gray-100">
              <div className="flex items-center gap-1 mb-4">
                ...
                <ButtonCstm variant="ghost" size="sm" className="gap-2 text-gray-600">
                  <MessageCircle className="h-5 w-5" />
                  <span className="font-medium">{currentPost.stats.comments}</span>
                </ButtonCstm>
              </div>
